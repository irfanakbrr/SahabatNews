@props([
    'type' => 'success', // success, error, warning, info
    'title' => '',
    'message' => '',
    'autohide' => true,
    'duration' => 5000
])

@php
$typeClasses = [
    'success' => [
        'container' => 'bg-green-500/20 border-green-500/50 text-green-300',
        'icon' => 'text-green-400',
        'svg' => '<path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>'
    ],
    'error' => [
        'container' => 'bg-red-500/20 border-red-500/50 text-red-300',
        'icon' => 'text-red-400',
        'svg' => '<path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>'
    ],
    'warning' => [
        'container' => 'bg-yellow-500/20 border-yellow-500/50 text-yellow-300',
        'icon' => 'text-yellow-400',
        'svg' => '<path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>'
    ],
    'info' => [
        'container' => 'bg-blue-500/20 border-blue-500/50 text-blue-300',
        'icon' => 'text-blue-400',
        'svg' => '<path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>'
    ]
];

$classes = $typeClasses[$type] ?? $typeClasses['info'];
$uniqueId = 'toast-' . uniqid();
@endphp

<div id="{{ $uniqueId }}" {{ $attributes->merge(['class' => 'mb-4 p-4 rounded-lg border backdrop-blur-sm transition-all duration-300 transform ' . $classes['container']]) }}>
    <div class="flex items-start">
        <svg class="w-5 h-5 mr-3 mt-0.5 flex-shrink-0 {{ $classes['icon'] }}" fill="currentColor" viewBox="0 0 20 20">
            {!! $classes['svg'] !!}
        </svg>
        
        <div class="flex-1 min-w-0">
            @if($title)
                <h4 class="font-semibold text-base mb-1">{{ $title }}</h4>
            @endif
            
            @if($message)
                <div class="text-sm opacity-90">{{ $message }}</div>
            @endif
            
            {{ $slot }}
        </div>
        
        @if($autohide)
            <button type="button" 
                    class="ml-3 flex-shrink-0 {{ $classes['icon'] }} hover:opacity-75 transition-opacity"
                    onclick="closeToast('{{ $uniqueId }}')"
                    aria-label="Close notification">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/>
                </svg>
            </button>
        @endif
    </div>
</div>

@if($autohide)
<script>
document.addEventListener('DOMContentLoaded', function() {
    const toast = document.getElementById('{{ $uniqueId }}');
    if (toast) {
        // Auto-hide after specified duration
        setTimeout(() => {
            closeToast('{{ $uniqueId }}');
        }, {{ $duration }});
    }
});

function closeToast(toastId) {
    const toast = document.getElementById(toastId);
    if (toast) {
        toast.style.opacity = '0';
        toast.style.transform = 'translateY(-10px) scale(0.95)';
        setTimeout(() => {
            if (toast.parentNode) {
                toast.parentNode.removeChild(toast);
            }
        }, 300);
    }
}
</script>
@endif 