...
<div class="relative z-1 flex h-screen w-full flex-col overflow-auto p-5 sm:p-10 bg-gradient-to-br from-purple-900 via-purple-600 to-pink-600">
    <div id="card" class="relative mx-auto my-auto w-full max-w-md shrink-0 overflow-hidden rounded-3xl border-t border-white/20 bg-gradient-to-t from-purple-100/10 to-purple-950/50 to-50% p-8 text-white shadow-2xl shadow-black/50 outline -outline-offset-1 outline-white/5 backdrop-blur-2xl">
        ...
        <div class="mb-6 text-center">
            ...
            <p class="text-purple-200 text-sm">Lupa Password?</p>
        </div>

        <div class="mb-6 text-sm text-purple-200">
            {{ __('Tidak masalah! Beri tahu kami alamat email Anda dan kami akan mengirimkan tautan reset password yang memungkinkan Anda membuat password baru.') }}
        </div>
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-4">
            ...
            <div class="flex items-center justify-between mt-6">
                <a href="{{ route('login') }}" class="text-sm text-purple-200 hover:text-white transition-colors">
                    {{ __('Kembali ke login') }}
                </a>

                <button type="submit" class="h-10 px-6 cursor-pointer rounded-md bg-gradient-to-r from-purple-600 to-pink-600 text-sm font-semibold text-white shadow-xl hover:from-purple-700 hover:to-pink-700 transition-all duration-300 transform hover:scale-[1.02]">
                    {{ __('Kirim Link Reset') }}
                </button>
            </div>
        </form>
